...
export function transitionRequest(request, actor, action, options = {}) {
  ...
  if (action === "approve" && actor.id === request.ownerId) {
    return denial("unauthorized", "Dual control prevents request owners from approving their own request.");
  }
  if (action === "approve" && request.evidence.some((item) => item.state !== "verified")) {
    return denial("evidence_incomplete", "Every required evidence item must be verified before approval.");
  }
  ...
  const event = {
    ...
    action: `request.${action === "request_changes" ? "changes_requested" : action}`,
