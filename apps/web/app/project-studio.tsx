    ...
    <main id="project-main" tabIndex={-1}>
      ...
      <section className={styles.directions} aria-labelledby="directions-heading">
        ...
        {candidates.length ? <ul className={styles.candidates}>{candidates.map((candidate, index) => <li key={candidate.id}>
          ...
          <details><summary>Craft and resilience</summary><dl><dt>Asset strategy</dt><dd>{candidate.asset_strategy}</dd><dt>Asset role</dt><dd>{candidate.asset_role}</dd><dt>Alternatives considered</dt><dd>{candidate.asset_alternatives}</dd><dt>Motion</dt><dd>{candidate.motion_rationale}</dd><dt>Interruption / reversal</dt><dd>{candidate.motion_interruption}</dd><dt>Performance budget</dt><dd>{candidate.motion_performance_budget}</dd><dt>Responsive</dt><dd>{candidate.responsive_strategy}</dd><dt>Reduced motion</dt><dd>{candidate.reduced_motion}</dd><dt>States</dt><dd>{candidate.states.join(" · ")}</dd></dl></details>
